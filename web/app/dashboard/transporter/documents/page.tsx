...
const DRIVER_CHECKLIST = [
  { item: 'Commercial Invoice', note: 'Keep 3 signed copies in the cab' },
  { item: 'Bill of Lading', note: 'Required at border for cargo release' },
  { item: 'Transit Bond', note: 'Customs hold until destination scan' },
  { item: 'Vehicle Roadworthiness Certificate', note: 'Inspected at border post' },
  { item: 'Driver\'s Yellow Fever Card', note: 'Required for DRC / Rwanda routes' },
];
...
export default function TransporterDocumentsPage() {
  ...
  function handleFile(file: File | null) {
    ...
    if (!allowed.includes(file.type)) { setFileError('Only PDF, JPG, or PNG files are accepted.'); return; }
    if (file.size > 5 * 1024 * 1024) { setFileError('File size must be under 5MB.'); return; }
    setFileError('');
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Cross-Border Documents</h2>
          <p className="text-sm text-gray-500 mt-0.5">
            Upload your vehicle and compliance documents for cross-border cargo runs
          </p>
        </div>
        <button
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-amber-800">No smartphone? Update status via USSD</p>
          <p className="text-sm text-amber-700 mt-0.5">
            Dial <strong>*384#</strong> → select "My Jobs" → "Update Document Status" — works on any phone without internet.
          </p>
        </div>
      </div>

      {/* Driver checklist panel */}
      {checklistOpen && (
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-5">
          <h3 className="font-semibold text-[#1E3A8A] mb-4 flex items-center gap-2">
            <Info size={15} /> Essential Documents for Cross-Border Runs
          </h3>
          <div className="space-y-2">
            {DRIVER_CHECKLIST.map(({ item, note }) => (
              <div key={item} className="flex items-start gap-3 bg-white rounded-lg border border-blue-100 px-4 py-3">
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-4">
          ...
          <button
            onClick={() => uploadedFile && selectedType && uploadMutation.mutate({ file: uploadedFile, type: selectedType as DocType })}
          ...
          {isLoading ? (
            ...
            <div className="p-10 text-center">
              ...
              <p className="text-xs text-gray-400 mt-1">
                {docs.length === 0
                  ? 'Upload your vehicle and compliance documents.'
