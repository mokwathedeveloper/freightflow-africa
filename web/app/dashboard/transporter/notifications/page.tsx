...
import { Bell, CheckCheck, Loader2, Truck, CheckCircle, AlertTriangle, Package, Info } from 'lucide-react';
...
const TABS = ['All', 'Unread', 'Jobs', 'System'] as const;
type Tab = typeof TABS[number];

function notifIcon(title: string) {
  const t = title.toLowerCase();
  if (t.includes('assign') || t.includes('accept') || t.includes('transit') || t.includes('pickup'))
    return { icon: Truck,         bg: 'bg-blue-50',   color: 'text-[#1E3A8A]' };
  if (t.includes('confirm') || t.includes('deliver'))
...
export default function TransporterNotificationsPage() {
  ...
  const filtered = all.filter((n) => {
    ...
    if (tab === 'Jobs')   return n.title.toLowerCase().includes('load') || n.title.toLowerCase().includes('job') || n.title.toLowerCase().includes('cargo') || n.title.toLowerCase().includes('transit') || n.title.toLowerCase().includes('deliver') || n.title.toLowerCase().includes('assign');
    ...
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Job updates, alerts, and system messages</p>
        </div>
        {unreadCount > 0 && (
          <button
          ...
          >
            {markAllMut.isPending ? <Loader2 className="animate-spin" size={13} /> : <CheckCheck size={15} />}
            Mark all as read
          </button>
        )}
      </div>
          ...
          <div className="divide-y divide-gray-100">
            {filtered.map((n) => {
              ...
              return (
                <div
                  key={n.id}
                  className={cn('flex items-start gap-4 px-5 py-4 transition-colors', !n.isRead && 'bg-blue-50/40')}
                >
                  <div className={cn('w-2 h-2 rounded-full mt-2.5 shrink-0', n.isRead ? 'bg-transparent' : 'bg-[#1E3A8A]')} />
