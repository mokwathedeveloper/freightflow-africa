...
// Tabs matching shared_pages.md: All | Loads | Tracking | System
...
function notifIcon(title: string) {
  ...
  if (t.includes('accept') || t.includes('assign'))
  ...
  if (t.includes('load') || t.includes('post') || t.includes('cargo'))
...
function isTracking(title: string) {
  ...
  return t.includes('transit') || t.includes('pickup') || t.includes('picked') ||
         ...
         t.includes('arrived') || t.includes('en route');
}
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Load updates, alerts, and system messages
          </p>
        </div>
        {unreadCount > 0 && (
          <button
            ...
            {filtered.map((n) => {
              ...
              return (
                <div
                  key={n.id}
                  className={cn(
                    'flex items-start gap-4 px-5 py-4 transition-colors hover:bg-gray-50/60 cursor-default',
